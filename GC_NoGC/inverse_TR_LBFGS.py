...
#                    m_misfit += assemble(conditional(abs(M_clusters - M_c) < eps, T - T_melt, 0.0) * dx) ** 2
    ...
    functional_file = "functional_TAO_TL_BQNKTL_orig.txt"
    solutions_vtk = VTKFile("solutions_TAO_TL_BQNKTL_orig.pvd")
    ...
    # Core algorithm
    # -------------------------------------------------------
    # bqnktl = bounded quasi-Newton Krylov trust-region
    #          with line-search fallback
    "tao_type": "bqnktr",
    ...
    # -------------------------------------------------------
    # Subproblem solver (Krylov)
    # -------------------------------------------------------
    "tao_bqnk_ksp_type": "stcg",   # Steihaug-Toint CG

    # -------------------------------------------------------
    # LMVM (L-BFGS) settings
    # -------------------------------------------------------
    # This is the KEY difference vs BNTR:
    # here LMVM is the actual Hessian approximation
    "tao_bqnk_pc_type": "lmvm",

    # Optional but recommended:
    "tao_bqnk_pc_lmvm_mat_lmvm_hist_size": 10,   # memory (L-BFGS pairs)
    ...
    def monitor(tao):
        ...
        try:
            ...
            try:
                delta = tao.getTrustRadius()
            except AttributeError:
                delta = float("nan")
            # if np.isnan(delta):
            #     # Fallback for some BNK versions: the 'step' value in status 
            #     # is often used for radius, but if that's 1.0, 
            #     # let's look for the internal 'trust' variable.
            #     delta = tao.getVariable("trust")
            # if its < 3:
            #     ls = tao.getLineSearch()
            #     ls.setInitialStepLength(1e-2)
            print(f"TAO Iteration: {its} | Functional: {f} | Function evaluations: {counter_func} | Gradient evaluations: {counter_grad} | Hessian evaluations: {counter_hess} | Gradient Norm: {res} | C-Norm: {cnorm} | Step Size: {step} | Trust Radius: {delta} | Initial Misfit: {initial_misfit} | Final Misfit: {final_misfit} | Elapsed time: {elapsed}")
            # Write functional and misfit values to a file (appending to avoid overwriting)
            if MPI.COMM_WORLD.Get_rank() == 0:        
                with open(functional_file, "a") as file:
                    file.write(f"TAO Iteration: {its} | Functional: {f} | Function evaluations: {counter_func} | Gradient evaluations: {counter_grad} | Hessian evaluations: {counter_hess} | Gradient Norm: {res} | C-Norm: {cnorm} | Step Size: {step} | Trust Radius: {delta} | | Initial Misfit: {initial_misfit} | Final Misfit: {final_misfit} | Elapsed time: {elapsed} \n")
            # Write VTK output:       
