...
#                    m_misfit += assemble(conditional(abs(M_clusters - M_c) < eps, T - T_melt, 0.0) * dx) ** 2
    ...
    functional_file = "functional_TAO_LS_BQNLS_orig.txt"
    solutions_vtk = VTKFile("solutions_TAO_LS_BQNLS_orig.pvd")
    ...
    solver = TAOSolver(minimisation_problem, {
        ...
        # -------------------------------------------------------
        "tao_type": "bqnls",                 # Bounded Quasi-Newton line search

        # -------------------------------------------------------
        # 2. L-BFGS / Limited-Memory Matrix (used inside BLMVM)
        # -------------------------------------------------------
        "tao_lmm_vectors": 40,                        # Number of stored (s,y) correction pairs; default: 5
        ...
        "tao_ls_gtol": 0.1,                          # Curvature (Wolfe) tolerance; default: 0.9
        # "tao_ls_stepmin": 1e-10,                     # Minimum step length allowed; default: 1e-20
        # "tao_ls_stepmax": 1e-3,                      # Maximum step length allowed; default: 1e10
        # "tao_ls_rtol": 1e-10,                        # Relative line-search tolerance; default: 1e-10
        # # "tao_ls_monitor": "",                      # Print line search progress; default: off
        "tao_ls_initstep": 1e-2,
