...
import patch_checkpoint_load  # import before opening the file
...
def inverse(alpha_T=1e0, alpha_u=1e-1, alpha_m=1e-1, alpha_d=1e-2, alpha_s=1e-1):
    ...
    # s = Constant(5.0)
    # delta = mu_eff - Constant(0.4)
    # mu_smooth = Constant(0.4) + delta * Constant(0.5) * (Constant(1.0) + tanh(s * delta))
    # mu = mu_smooth
...
#                    m_misfit += assemble(conditional(abs(M_clusters - M_c) < eps, T - T_melt, 0.0) * dx) ** 2
        ...
        if alpha_u > 0:
            ...
            # u_surface_misfit += assemble(Constant(alpha_u) * dot(u - uobs, u - uobs) * ds_t)
    ...
    if alpha_T > 0:
        ...
        # T_misfit = assemble(Constant(alpha_T) * (T - Tobs) ** 2 * dx)
    ...
    if alpha_s > 0:
        ...
        # smoothing = assemble(Constant(alpha_s) * dot(grad(T_0 - Taverage), grad(T_0 - Taverage)) * dx)
    ...
    #     delta_temp
    # )
    # log(f"Taylor test minconv: {minconv}")
    ...
    functional_file = "functional_NK.txt"
    ...
    def record_post_hess(*args):
        ...
        # print(f"hessian_cb_post args: {[type(a) for a in args]}", flush=True)
        # print(f"hessian_cb_post args[0]: {args[0]}", flush=True)
        # # Same Riesz map fix for the HVP result -- reduced_functional.hessian()
        # # also returns a Cofunction; ROL's CG must receive a primal Function.
        # h_riesz = hessian_action[0].riesz_representation(riesz_map="L2")
        # hessian_action[0].assign(h_riesz)
        # return hessian_action

    
    def record_pre_func(*args):
        # Reset Stokes solution to zero before each new forward solve.
        # This prevents the adjoint backward pass from corrupting the
        # SNES initial guess for subsequent functional evaluations.
    ...
    def record_post_grad(checkpoint, derivatives, values, *args):
        ...
        # g_riesz = derivatives[0].riesz_representation(riesz_map="L2")
        # derivatives[0].assign(g_riesz)
        return derivatives
    ...
    #     # "tao_blmvm_eps": 1e-8,                        # Bound feasibility tolerance; default: 1e-8
    #     # "tao_bounded_type": "project",                  # Bound enforcement ('clip'|'project'); default: 'clip'


    #     # -------------------------------------------------------
    #     # 2. L-BFGS / Limited-Memory Matrix (used inside BLMVM)
    #     # -------------------------------------------------------
    #     "tao_lmm_vectors": 10,                        # Number of stored (s,y) correction pairs; default: 5
    ...
    #     # "tao_ls_armijo_alpha": 1e-4,
    #     "tao_ls_ftol": 1e-4,                         # Armijo sufficient decrease parameter; default: 1e-4
    #     "tao_ls_gtol": 0.6,                          # Curvature (Wolfe) tolerance; default: 0.9
    #     "tao_ls_stepmin": 1e-8,                     # Minimum step length allowed; default: 1e-20
    #     "tao_ls_stepmax": 1,                      # Maximum step length allowed; default: 1e10
    #     # "tao_ls_rtol": 1e-10,                        # Relative line-search tolerance; default: 1e-10
    #     "tao_ls_monitor": "",                      # Print line search progress; default: off
    #     "tao_ls_initstep": 0.26,

    #     # -------------------------------------------------------
    #     # 4. Stopping Criteria
    #     # -------------------------------------------------------
    #     "tao_gatol": 1e-7,                           # Absolute gradient tolerance; default: 1e-8
    #     "tao_grtol": 1e-7,                           # Relative gradient tolerance; default: 1e-8
    ...
    # from pyadjoint.optimization.rol_solver import ROLObjective
    # import ROL as _ROL

    # Patch ROLObjective.update to skip forward solves on Temp/scratch vectors,
    # which Newton-Krylov's CG triggers during workspace initialisation.
    # The unpatched version runs self.rf(x.dat) on every update() call including
    # Temp flag calls on cloned CG scratch vectors -- corrupting tape state.
    # _original_update = ROLObjective.update

    # def _patched_update(self, x, flag, iteration):
    #     if hasattr(_ROL, "UpdateType") and isinstance(flag, _ROL.UpdateType):
    #         # Only run forward solve for Initial, Trial, Accept -- NOT Temp.
    #         # Temp is used by Newton-Krylov CG for scratch workspace vectors.
    #         if flag in [_ROL.UpdateType.Initial, _ROL.UpdateType.Trial]:
    #             self._val = self.rf(x.dat)
    #             self._tape_trial = self.rf.tape.checkpoint_block_vars(
    #                 self.rf.controls)
    #         elif flag == _ROL.UpdateType.Accept:
    #             self._cache = self._val
    #             self._tape_cache = self._tape_trial
    #         elif flag == _ROL.UpdateType.Revert:
    ...
    #                 self._cache = self._val
    #                 self._tape_cache = self._tape_trial
    #         # UpdateType.Temp: do nothing -- scratch vector, no forward solve
    #         self._flag = flag
    #     else:
    #         # ROL 1.x integer flag path -- same guard: skip if flag == -1 (Temp)
    #         if flag != -1:
    #             self._val = self.rf(x.dat)

    # ROLObjective.update = _patched_update
    ...
    class StatusTest(ROL.StatusTest):
        def check(self, status):
            nonlocal iteration  # FIX: was `global iteration`, which referenced a
                                # nonexistent module-level name and would have
                                # either raised NameError or silently created a
                                # disconnected global on first write.

            # delta_t = Constant(4e-6)

            # FIX: Tic.block_variable.checkpoint / T.block_variable.checkpoint
            # can legitimately be None on the call(s) where SingleMemoryStorageSchedule
            # hasn't retained a forward-solve checkpoint yet (e.g. immediately after
            # the Taylor test, before ROL has driven a fresh functional evaluation
            # through reduced_functional). Guard before assembling.
            ic_checkpoint = Tic.block_variable.checkpoint
            final_checkpoint = T.block_variable.checkpoint

            if ic_checkpoint is None or final_checkpoint is None:
                log(
                    f"StatusTest.check(): checkpoint not yet available "
                    f"(Tic checkpoint={'set' if ic_checkpoint is not None else 'None'}, "
                    f"T checkpoint={'set' if final_checkpoint is not None else 'None'}); "
                    f"skipping misfit computation for this call."
                )
                if MPI.COMM_WORLD.Get_rank() == 0:
                    with open(functional_file, "a") as f:
                        f.write(
                            f"Iteration: {iteration} \n"
                            f"Checkpoint not available yet; skipped misfit computation\n"
                        )
                iteration = iteration + 1
                return super().check(status)

            initial_misfit = assemble((ic_checkpoint - Tic_ref) ** 2 * dx)
            final_misfit = assemble((final_checkpoint - Tobs) ** 2 * dx)
            ...
            # Write VTK output:
            solution_IC.assign(ic_checkpoint)
            solution_final.assign(final_checkpoint)
    ...
    #     initial_misfit = initial_misfit_values[-1] if initial_misfit_values else float('nan')
    #     current_tic = control.block_variable.saved_output
    #     if current_tic is not None:
    #         solution_IC.assign(current_tic)
    ...
    #     objective += d_contribution
    #     log(f"D_contribution: {d_contribution}")
    #     log(f"Objective (TUD): {objective}")
    #     smoothing = assemble(Function(R, name="alpha_s").assign(float(alpha_s)) * dot(grad(T_0 - Taverage), grad(T_0 - Taverage)) * dx)    
    #     norm_smoothing = assemble(dot(grad(Tobs - Taverage), grad(Tobs - Taverage)) * dx)
    #     s_contribution = norm_obs * smoothing / norm_smoothing
    #     objective += s_contribution
    #     log(f"S_contribution: {s_contribution}")        
    #     log(f"Objective (TUDS): {objective}")
    ...
    # # solver.tao.setFromOptions()
    # # ls = tao.getLineSearch()
    # # ls.setInitialStepLength(1e-4)          
    # # ls.setMaximumFunctionEvaluations(50) 
    # T_opt = solver.solve()
