...
if [[ "${RUN_ID}" -lt "${MAX_RUNS}" ]]; then
    ...
    qsub -W depend=afterok:${PBS_JOBID} -v RUN_ID=${next_run},MAX_RUNS=${MAX_RUNS} ../job_bqnls.sh
    popd
fi
...
mpiexec -np $PBS_NCPUS python ../mpl_wrapper.py ../inverse_BQNLS.py &> output_BQNLS.dat
next_run=$((RUN_ID + 1))
