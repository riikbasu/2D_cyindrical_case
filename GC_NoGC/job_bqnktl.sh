...
if [[ "${RUN_ID}" -lt "${MAX_RUNS}" ]]; then
    ...
    qsub -W depend=afterok:${PBS_JOBID} -v RUN_ID=${next_run},MAX_RUNS=${MAX_RUNS} ../job_bqnktl.sh
    popd
fi
...
mpiexec -np $PBS_NCPUS python ../mpl_wrapper.py ../inverse_BQNKTL.py &> output_BQNKTL.dat
next_run=$((RUN_ID + 1))
