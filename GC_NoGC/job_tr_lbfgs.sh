...
if [[ "${RUN_ID}" -lt "${MAX_RUNS}" ]]; then
    ...
    qsub -W depend=afterok:${PBS_JOBID} -v RUN_ID=${next_run},MAX_RUNS=${MAX_RUNS} ../job_tr_lbfgs.sh
    popd
fi
...
mpiexec -np $PBS_NCPUS python ../mpl_wrapper.py ../inverse_TR_LBFGS.py &> output_TR_LBFGS.dat
next_run=$((RUN_ID + 1))
