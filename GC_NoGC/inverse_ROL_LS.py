...
#                    m_misfit += assemble(conditional(abs(M_clusters - M_c) < eps, T - T_melt, 0.0) * dx) ** 2
    ...
    functional_file = "functional_ROL_LS_QNM_orig.txt"
    solutions_vtk = VTKFile("solutions_ROL_LS_QNM_orig.pvd")
    ...
    def record_post_func(func_value, *args):
        ...
        current_tic = controls if not isinstance(controls, list) else controls[0]
    ...
    class StatusTest(ROL.StatusTest):
        def check(self, status):
            ...
            if T.block_variable.checkpoint is not None:
