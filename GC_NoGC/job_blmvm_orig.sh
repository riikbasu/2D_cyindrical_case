...
if [[ "${RUN_ID}" -lt "${MAX_RUNS}" ]]; then
    ...
    qsub -W depend=afterok:${PBS_JOBID} -v RUN_ID=${next_run},MAX_RUNS=${MAX_RUNS} ../job_blmvm_orig.sh
    popd
fi
...
module load firedrake/Riik_tests
...
mpiexec -np $PBS_NCPUS python ../mpl_wrapper.py ../inverse_BLMVM_orig.py &> output_BLMVM_orig.dat
next_run=$((RUN_ID + 1))
