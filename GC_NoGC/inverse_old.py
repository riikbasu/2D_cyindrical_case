...
def generate_inverse_problem(alpha_T=1.0, alpha_u=-1, alpha_m=-1, alpha_d=-1, alpha_s=-1, rol_checkpoint_dir=None, restore_iteration=0):
    ...
    for timestep in tape.timestepper(iter(range(min_timesteps, max_timesteps))):
        ...
        if alpha_m > 0:
            if melt_approx == False:  # Full melting info
                ...
                m_misfit += assemble(Function(R, name="alpha_m").assign(float(alpha_m)/(max_timesteps - min_timesteps)) * M_mask * (T - T_melt) ** 2 * dx)
                ...
                for c in range(max_clusters):
                    M_c = float(c + 1)
                    m_misfit += assemble(Function(R, name="alpha_m").assign(float(alpha_m)/(max_timesteps - min_timesteps)) * conditional(abs(M_clusters - M_c) < eps, T - T_melt, 0.0) * dx) ** 2
    ...
    if alpha_m > 0:
        ...
        norm_m = assemble(dot(M_mask_final * T_melt_final, M_mask_final * T_melt_final) * dx)                
