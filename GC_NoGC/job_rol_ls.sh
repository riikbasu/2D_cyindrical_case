...
if [[ "${RUN_ID}" -lt "${MAX_RUNS}" ]]; then
    ...
    qsub -W depend=afterok:${PBS_JOBID} -v RUN_ID=${next_run},MAX_RUNS=${MAX_RUNS} ../job_rol_ls.sh
    popd
fi
...
mpiexec -np $PBS_NCPUS python ../mpl_wrapper.py ../inverse_ROL_LS.py &> output_ROL_LS.dat
next_run=$((RUN_ID + 1))
